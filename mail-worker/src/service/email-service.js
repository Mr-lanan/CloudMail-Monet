...
const emailService = {
	...
	async send(c, params, userId) {
		...
		const { r2Domain, send, domainList } = settingRow;
		...
		let providerResult = { id: null, provider: 'internal' };

		//存在站外收件人时，通过已配置的服务商自动故障切换发送
		if (!allInternal) {
			const sendForm = {
				fromName: name,
			...
			try {
				providerResult = await mailProviderService.send(c, settingRow, sendForm);
			} catch (error) {
		...
		emailData.resendEmailId = providerResult.id;
		emailData.sendProvider = providerResult.provider;
