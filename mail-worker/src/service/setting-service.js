...
const settingService = {
	...
	async query(c) {
		...
		setting.providerPriority = normalizePriority(setting.providerPriority);
	...
	async get(c, showSiteKey = false) {
		...
		const providerConfigs = await mailProviderConfigService.decode(c, cachedSetting.mailProviders, cachedSetting.resendTokens);
		settingRow.mailProviders = mailProviderConfigService.mask(providerConfigs);
	...
	async set(c, params) {
		...
		if (params.mailProviders) {
			update.mailProviders = await mailProviderConfigService.mergeAndEncode(
				c,
				settingData.mailProviders,
				settingData.resendTokens,
				params.mailProviders
			);
			update.resendTokens = '{}';
		}

		if (params.resendTokens) {
			const resendTokens = { ...settingData.resendTokens, ...params.resendTokens };
		...
		if (Array.isArray(update.emailPrefixFilter)) update.emailPrefixFilter = update.emailPrefixFilter + '';
		if (Array.isArray(update.providerPriority)) update.providerPriority = normalizePriority(update.providerPriority).join(',');
