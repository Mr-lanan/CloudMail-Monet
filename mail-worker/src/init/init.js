...
const dbInit = {
	...
	async v3DB(c) {
		const sqlList = [
			`ALTER TABLE setting ADD COLUMN mail_providers TEXT NOT NULL DEFAULT '';`,
			`ALTER TABLE setting ADD COLUMN provider_priority TEXT NOT NULL DEFAULT 'resend,mailersend,brevo,postmark,ses';`,
			`ALTER TABLE email ADD COLUMN send_provider TEXT NOT NULL DEFAULT '';`
		];
	...
	async v1_6DB(c) {
		...
		const ADD_COLUMN_SQL_LIST = [
			...
			`ALTER TABLE setting ADD COLUMN notice_title TEXT NOT NULL DEFAULT 'CloudMail Monet';`,
	...
	async intDB(c) {
		...
		try {
			await c.env.db.prepare(`
			  ...
			  SELECT 0, 0, 0, 0, 'CloudMail Monet', 0, 1, 1
